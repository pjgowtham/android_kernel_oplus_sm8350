...
extern bool g_oplus_need_lp_mode;
...
int oplus_display_panel_get_id(void *buf)
{
	...
	if (get_oplus_display_power_status() == OPLUS_DISPLAY_POWER_ON) {
		...
		if (1 == display_id) {
			...
			if (!display) {
				printk(KERN_INFO "oplus_display_get_panel_get_id and second display is null");
				return -1;
			}
...
int oplus_display_set_cabc_status(void *buf)
{
	...
	if((get_oplus_display_power_status() == OPLUS_DISPLAY_POWER_ON && 0 != bl_lvl_backup)
		|| (get_oplus_display_power_status() == OPLUS_DISPLAY_POWER_ON && g_cabc_recover && (0 != bl_lvl_backup))) {
		mutex_lock(&display->display_lock);
