...
bool oplus_vooc_wake_voocphy_service_work(struct oplus_voocphy_manager *chip, int request)
{
	...
	if (chip->parallel_charge_support) {
		schedule_delayed_work_on(chip->service_work_cpu_id, &chip->voocphy_service_work, 0);
	} else {
...
int oplus_voocphy_parse_batt_curves(struct oplus_voocphy_manager *chip)
{
	...
	rc = of_property_read_u32(node, "qcom,service_work_cpu_id",
	                          &chip->service_work_cpu_id);
	if (rc) {
		chip->service_work_cpu_id = 0;
	} else {
		voocphy_info("qcom,service_work_cpu_id is %d\n", chip->service_work_cpu_id);
	}
