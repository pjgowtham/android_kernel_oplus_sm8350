				...
				const struct i2c_device_id *id)
{
	...
	chg_err(" batt_temp:%d, sub_batt_temp:%d\n", batt_temp, sub_batt_temp);

	chg_err("%s: ###############S2ASL01 Switching Driver Loading\n", __func__);
	if (batt_temp == -400 || sub_batt_temp == -400) {
		for(retry = 0;retry < 3;retry++) {
			batt_temp = oplus_gauge_get_batt_temperature();
			sub_batt_temp = oplus_gauge_get_sub_batt_temperature();
			if (batt_temp == -400 || sub_batt_temp == -400) {
				chg_err("gauge i2c error! batt_temp:%d, sub_batt_temp:%d\n", batt_temp, sub_batt_temp);
				error_status = SWITCH_ERROR_OVER_VOLTAGE;
			} else {
				error_status = 0;
				chg_err(" batt_temp:%d, sub_batt_temp:%d, error_status:%d\n", batt_temp, sub_batt_temp, error_status);
				break;
			}
		}
	}
